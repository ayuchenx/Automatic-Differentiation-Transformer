...
class AddOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        # print('add 1', input_values[0].shape)
        # print('add 2', input_values[1].shape)
        # print('add output', (input_values[0] + input_values[1]).shape)
...
class AddByConstOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        # print('add by const', input_values[0].shape)
...
class MulOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(input_values) == 2
        # print(f"mulop 1: {input_values[0].shape}")
        # print(f"mulop 2: {input_values[1].shape}")
        # print(f"mulop Output Shape: {(input_values[0].mul(input_values[1])).shape})")
        return input_values[0] * input_values[1]
...
class MulByConstOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(input_values) == 1
        # print('mul by const', input_values[0].shape)
        return input_values[0] * node.constant
...
class SubOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(input_values) == 2
        # print('sub 1', input_values[0].shape)
        # print('sub 2', input_values[1].shape)
        return input_values[0] - input_values[1]
...
class SumOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        # print(f"SumOp Output: {(input_values[0].sum(dim=node.dim, keepdim=node.keepdim)).shape})")
...
class ExpandAsOp3d(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        if input_tensor.ndim == 2 and input_tensor.size(-1) == target_tensor.size(-1):
            res = input_tensor.unsqueeze(1).expand_as(target_tensor)
        else:
            res = input_tensor.unsqueeze(-1).expand_as(target_tensor)
        return res
...
class LogOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        # print('log', input_values[0].shape)
...
class BroadcastOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        """Return the broadcasted tensor."""
        assert len(input_values) == 1
        # print('pre broadcast', input_values[0].shape)
        # print('pre broadcast', len(input_values[0].expand(node.attrs["target_shape"])))
...
class DivOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(input_values) == 2
        """TODO: your code here"""
        # print('div 1', input_values[0].shape)
        # print('div 2', input_values[1].shape)
        return input_values[0] / input_values[1]
...
class DivByConstOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(input_values) == 1
        """TODO: your code here"""
        # print('div by const', input_values[0].shape)
        return input_values[0] / node.constant
...
class TransposeOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        # print('transpose', input_values[0].shape)
        # print('tranpose output', torch.transpose(input_values[0], node.dim0, node.dim1).shape)
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        """TODO: your code here"""
        return [transpose(output_grad, -1, -2)]
...
class MatMulOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(input_values) == 2
        """TODO: your code here"""
        # print('matmul 1', input_values[0].shape)
        # print('matmul 2', input_values[1].shape)
        # print('matmul output', (input_values[0]@input_values[1]).shape)
...
class SoftmaxOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(input_values) == 1
        """TODO: your code here"""
        max_val, _ = torch.max(input_values[0], dim = node.dim, keepdim = True)
        numerator = torch.exp(input_values[0] - max_val)
        denominator = torch.sum(numerator, dim = node.dim, keepdim = True)
        # print('softmax', input_values[0].shape)
        # print('softmax output', (numerator / denominator).shape)
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        """TODO: your code here"""
        soft = softmax(node.inputs[0], node.dim)
        summed = sum_op(soft * output_grad, dim = node.dim, keepdim = True)
        return [soft * (output_grad - summed)]
...
class LayerNormOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(input_values) == 1
        """TODO: your code here"""
        input_vals = input_values[0]     
        dims = list(range(-len(node.normalized_shape), 0))   
        mean = torch.mean(input_vals, dim = dims, keepdim = True)
        variance = torch.var(input_vals, dim = dims, keepdim = True, unbiased = False)
        # print('layernorm', input_values[0].shape)
        # print('layernorm output', ((input_vals - mean) / torch.sqrt(variance + node.eps)).shape)
        return (input_vals - mean) / torch.sqrt(variance + node.eps)
    ...
    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        ...
        mean_val = mean(x, dim = dims, keepdim = True)
        variance = power(sub(x, mean_val), 2)
        variance = mean(variance, dim = dims, keepdim = True)
        
        g_mean = mean(output_grad, dim = dims, keepdim = True)
        another_mean = mean(mul(output_grad, sub(x, mean_val)), dim = dims, keepdim = True)
        
        res = sub(output_grad, g_mean)
        t1 = div(sub(x, mean_val), add_by_const(variance, node.eps))
...
class ReLUOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        ...
        assert len(input_values) == 1
        """TODO: your code here"""
        # print('relu', input_values[0].shape)
        # print('relu output', torch.maximum(torch.tensor(0), input_values[0]).shape)
        return torch.maximum(torch.tensor(0), input_values[0])

    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        """Given gradient of ReLU node, return partial adjoint to input."""
        """TODO: your code here"""
        res = greater(node.inputs[0], zeros_like(node.inputs[0]))
        return [output_grad * res]
...
class SqrtOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        assert len(input_values) == 1
        """TODO: your code here"""
        # print('sqrt', input_values[0].shape)
        return torch.sqrt(input_values[0])

    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        """TODO: your code here"""
        return [output_grad * mul_by_const(power(node.inputs[0], -1/2), 0.5)]
...
class PowerOp(Op):
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        assert len(input_values) == 1
        """TODO: your code here"""
        # print('pow', input_values[0].shape)
...
class MeanOp(Op):
    ...
    def __call__(self, node_A: Node, dim: tuple, shape: tuple = None, keepdim: bool = False) -> Node:
        return Node(
            inputs=[node_A],
            op=self,
            attrs={"dim": dim, "shape": shape, "keepdim": keepdim},
    ...
    def compute(self, node: Node, input_values: List[torch.Tensor]) -> torch.Tensor:
        assert len(input_values) == 1
        """TODO: your code here"""
        # print('premean', input_values[0].shape)
        res =  torch.mean(input_values[0], dim = node.dim, keepdim = node.keepdim)
        # print('postmean', res.shape)
        return res

    def gradient(self, node: Node, output_grad: Node) -> List[Node]:
        """TODO: your code here"""
        total = sum_op(ones_like(node.inputs[0]), dim = node.dim, keepdim = node.keepdim)
        return [ones_like(node.inputs[0]) * div(output_grad, total)]
...
class Evaluator:
    ...
    def run(self, input_values: Dict[Node, torch.Tensor]) -> List[torch.Tensor]:
        ...
        """
        """TODO: your code here"""
        sorted = topological_sort(self.eval_nodes)
        ...
        for node in sorted:
            ...
            if node in input_values:
                node_vals[node] = input_values[node]
                
            else:
                if not node.inputs:
                    raise ValueError(f"value of input node {node} is not given in the dictionary")
                
                inputs = [node_vals[input_node] for input_node in node.inputs]
                # print('-------------------------------------------')
                # print(node)
                node_vals[node] = node.op.compute(node, inputs)
        
        res = [node_vals[node] for node in self.eval_nodes]
...
def gradients(output_node: Node, nodes: List[Node]) -> List[Node]:
    ...
    """
    """TODO: your code here"""
    sorted = topological_sort([output_node])
    ...
    for node in reversed(sorted):
        ...
        if not isinstance(input_grads, list):
            input_grads = [input_grads]
            
        for input_node, input_grad in zip(node.inputs, input_grads):
            if input_node in grads:
                grads[input_node] += input_grad
            else:
                grads[input_node] = input_grad
    # for node in nodes:
    #     print(grads.get(node))
            
    return [grads.get(node, zeros_like(node)) for node in nodes]
